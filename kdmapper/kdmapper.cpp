...
bool ResolveImports(portable_executable::vec_imports imports) {
	for (const auto& current_import : imports) {
		...
		for (auto& current_function_data : current_import.function_datas) {
			ULONG64 function_address = new_driver::GetKernelModuleExport(Module, current_function_data.name);

			if (!function_address) {
				//Lets try with ntoskrnl
				if (Module != new_driver::ntoskrnlAddr) {
					function_address = new_driver::GetKernelModuleExport(new_driver::ntoskrnlAddr, current_function_data.name);
					if (!function_address) {
#if !defined(DISABLE_OUTPUT)
...
ULONG64 kdmapper::MapDriver(BYTE* data, ULONG64 param1, ULONG64 param2, bool free, bool destroyHeader, AllocationMode mode, bool PassAllocationAddressAsFirstParam, mapCallback callback, NTSTATUS* exitCode) {
	...
	{
		kernel_image_base = new_driver::MmAllocateIndependentPagesEx(image_size);
	}
	else { // AllocatePool by default
		kernel_image_base = new_driver::AllocatePool(nt::POOL_TYPE::NonPagedPool, image_size);
	}
	...
	do {
		...
		if (!new_driver::WriteMemory(realBase, (PVOID)((uintptr_t)local_image_base + (destroyHeader ? TotalVirtualHeaderSize : 0)), image_size)) {
		...
		if (mode == AllocationMode::AllocateIndependentPages)
		{
			...
			for (int i = 0; i < nt_headers->FileHeader.NumberOfSections; i++) {
				...
				if (!new_driver::MmSetPageProtection(secAddr, secSize, prot)) {
		...
		NTSTATUS status = 0;
		if (!new_driver::CallKernelFunction(&status, address_of_entry_point, (PassAllocationAddressAsFirstParam ? realBase : param1), param2)) {
		...
		if (free) {
			...
			if (mode == AllocationMode::AllocateIndependentPages)
			{
				free_status = new_driver::MmFreeIndependentPages(realBase, image_size);
			}
			else {
				free_status = new_driver::FreePool(realBase);
			}
	...
	if (mode == AllocationMode::AllocateIndependentPages)
	{
		free_status = new_driver::MmFreeIndependentPages(kernel_image_base, image_size);
	}
	else {
		free_status = new_driver::FreePool(kernel_image_base);
	}
